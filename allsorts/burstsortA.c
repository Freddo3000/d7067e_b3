    ...
    for( i=0 ; i<scnt ; i++ )
    {
        ...
        else /* bucket already created */
        {
            ...
            { 
                ...
                if ((new = (TRIE *) calloc(1, sizeof(TRIE))) == NULL)
                {
                    ERR_PRINT_OUT_OF_MEMORY;
                    EXIT;
                }
                ...
                /* burst ...*/
                 ...
                 for(j=0; j < currcounts; j++) 
                 {
                     ...
                     if(new->counts[cc] < 1)
                     {
                         ...
                         */
                         if (cc == NULLTERM)
                         {
                             if ((new->ptrs[cc] = (char **) calloc(THRESHOLD, sizeof(char *))) == NULL)
                             {
                                 ERR_PRINT_OUT_OF_MEMORY;
                                 EXIT;
                             }
                             new->nulltailptr = new->ptrs[cc];        /* point to the first cell of the bucket */
                             *new->nulltailptr = (char *) curr->ptrs[c][j]; /* insert the string */
                             new->nulltailptr++;                       /* point to next cell */
                             new->counts[cc]++;                        /* increment count of items */
                         }
                         else
                         {
                             if ((new->ptrs[cc] = (char **) calloc(bucket_inc[1], sizeof(char *))) == NULL)
                             {
                                 ERR_PRINT_OUT_OF_MEMORY;
                                 EXIT;
                             }
                             ...
                             */
                             new->ptrs[cc][new->counts[cc]++] = (char *) curr->ptrs[c][j];
                             new->levels[cc]++;
                         }
                     }
                     else
                     {
                         ...
                         if (cc == NULLTERM)
                         {
                             *new->nulltailptr = (char *) curr->ptrs[c][j]; /* insert the string */
                             ...
                             /* check if the bucket is reaching the threshold */
                             if ( (new->counts[cc] % THRESHOLDMINUSONE) == 0)
                             {
                                 if ((*new->nulltailptr = (char *) calloc(THRESHOLD, sizeof(char *))) == NULL)
                                 {
                                     ERR_PRINT_OUT_OF_MEMORY;
                                     EXIT;
                                 }
                                     ...
                                     /* point to the first cell in the new array */
                                     new->nulltailptr = (char **) *new->nulltailptr;
                             }
                         }
                         else
                         {
                             ...
                             ** check if the number of items in the bucket is above a threshold
                             */
                             newcounts = new->counts[cc];
                             if (newcounts < THRESHOLD && newcounts > (bucket_inc[new->levels[cc]] - 1) )
                                 if((new->ptrs[cc]  = (char **) realloc(new->ptrs[cc], bucket_inc[++new->levels[cc]] * sizeof(char *))) == NULL)
                                 {
                                    ERR_PRINT_OUT_OF_MEMORY;
                                    EXIT;
                                 }
                 ...
                 free(curr->ptrs[c]);
                 curr->ptrs[c] = (char **) new;	/* old pointer points to the new trie node		*/
                 curr->counts[c] = -1;		/* flag to indicate pointer to trie node and not bucket	*/ 
                 curr = new;			/* used to burst recursive, so point curr to new	*/
...
void
burstsortA(string strings[], int scnt)
{
    TRIE	*root;
